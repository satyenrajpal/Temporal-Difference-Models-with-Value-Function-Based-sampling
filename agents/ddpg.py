...
class DDPGAgent(Agent):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        #Expectation of Q values as the loss, update the actor
    ...
    def select_action(self, state,goal_state,tau):
        ...
        action = self.actor.predict_on_batch([state_batch,goal_batch,tau_batch]).flatten()
    ...
    def forward(self, observation,goal_state,tau):
        # Select an action. 
        # state = self.memory.get_recent_state(observation) #WTF is this?
        state = [observation]                                                                                            #B
        action = self.select_action(state,goal_state,tau) #Check what is state and how to accomodate tau
        # if self.processor is not None:    #^ Should get this in some way!
        #     action = self.processor.process_action(action)

        # Book-keeping.
        # self.recent_observation = observation
        # self.recent_action = action
        # self.recent_goal_state = goal_state
        # self.recent_tau=tau
        return action
    ...
    def backward(self, reward, terminal=False):
        ...
        # if self.step % self.memory_interval == 0:
            ...
            #  reward, terminal,self.recent_goal_state,self.recent_tau, training=self.training)
        ...
        if can_train_either and self.step % self.train_interval == 0:
            experiences = self.memory.sample(self.batch_size) #######IMPORTANT!
            ...
            for e in experiences:
                ...
                goal_batch.append(e.goal_state)
                achieved_goal_batch.append(e.achieved_goal)
                terminal1_batch.append(0. if e.terminal1 else 1.)
                # tau_batch.append(e.tau)

            # Prepare and validate parameters.
            state0_batch = self.process_state_batch(state0_batch) #returns np.array
            state1_batch = self.process_state_batch(state1_batch)
            terminal1_batch = np.array(terminal1_batch)
            reward_batch = np.array(reward_batch)
            action_batch = np.array(action_batch)
            goal_batch = self.process_state_batch(goal_batch)
            achieved_goal_batch= self.process_state_batch(achieved_goal_batch)#returns np.array
            ...
            if self.step > self.nb_steps_warmup_critic:
                target_actions = self.target_actor.predict_on_batch([state1_batch,goal_batch,tau_batch]) #Change this!
                assert target_actions.shape == (self.batch_size, self.nb_actions)
                #WHY THIS!???? changed this to accomodate for more inputs
                # if len(self.critic.inputs) >= 3:
                #     state1_batch_with_action = state1_batch[:]
                # else:
                state1_batch_with_action = [state1_batch,goal_batch]
                ############################################
                ...
                discounted_reward_batch *= terminal1_batch #Will have to change this also, I guess?
                ...
                # else:
                state0_batch_with_action = [state0_batch,goal_batch]
            ...
            if self.step > self.nb_steps_warmup_actor:
                ...
                #     inputs = state0_batch[:]
                # else:
                inputs = [state0_batch,goal_batch,tau_batch]
                if self.uses_learning_phase:
